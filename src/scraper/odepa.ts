...
const MARKET_MAP: Record<string, { mercado: string; region: string; id_region: number }> = {
  ...
  'Mapocho Vta.dir': {
    mercado: 'Mapocho venta directa de Santiago',
    region: 'Región Metropolitana de Santiago',
    id_region: 13,
  },
  'Macroferia Talca': {
    mercado: 'Macroferia Regional de Talca',
    region: 'Región del Maule',
    id_region: 7,
  },
  Femacal: { mercado: 'Femacal de La Calera', region: 'Región de Valparaíso', id_region: 5 },
  'La Palmera': {
    mercado: 'Terminal La Palmera de La Serena',
    region: 'Región de Coquimbo',
    id_region: 4,
  },
  Solcoagro: { mercado: 'Solcoagro de Ovalle', region: 'Región de Coquimbo', id_region: 4 },
  'Vega Monumental': {
    mercado: 'Vega Monumental Concepción',
    region: 'Región del Biobío',
    id_region: 8,
  },
  'Lagunita Pto.Montt': {
    mercado: 'Feria Lagunitas de Puerto Montt',
  ...
  Agrochillan: {
    mercado: 'Terminal Hortofrutícola Agro Chillán',
    region: 'Región de Ñuble',
    id_region: 16,
  },
  Agronor: {
    mercado: 'Agrícola del Norte S.A. de Arica',
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    for (let i = dataStartRow; i < rows.length; i++) {
      ...
      records.push({
        ...
        precio_kg_unidad_comercializacion:
          kgUnit > 0 ? Math.round((precioMin / kgUnit) * 100) / 100 : 0,
