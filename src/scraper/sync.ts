...
export async function runWeeklySync(delayMs = 600): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[sync] Weekly sync: ${weekDates.length} dates from ${monday.toISOString().slice(0, 10)} to ${today.toISOString().slice(0, 10)}`
    );
...
export async function runBackfill(delayMs = 600): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[sync] Backfill: ${dates.length} dates from ${lastDate.toISOString().slice(0, 10)}`
    );
